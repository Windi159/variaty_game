...
# CONFIG # 설정 부분. 게임에 필요한 열거형 및 클래스들을 정의
...
# 게임 보드에 있는 각 위치 노드를 나타내는 클래스입니다.
...
class BoardGraph(object):
    ...
    # 게임 보드 노드 간의 전이 규칙을 정의
    _TRANSITIONS = {
        # from_node: (dest_node | outcome for outcome in _OUTCOMES)
        # _OUTCOMES order is assumed and respected for all _TRANSITIONS!

        # No piece on board # 게임 보드에 말이 없을 때
        OUT: (E1, E2, (E3, R4), (E4, R3), (NE, CC)),

        # center and corners # 중앙 및 코너 위치
        NE: (N1, N2, (N3, G1), (N4, G2), (NW, CC)),
        NW: (W1, W2, W3, W4, SW),
        SW: (S1, S2, S3, S4, SE),
        SE: (WON, WON, WON, WON, WON),
        CC: ((G3, R2), (G4, R1), (S3, W3), (S4, W4), (SE, SW)),
        
        # straight-line clockwise 1s # 시계 방향 1로 이동
        E1: (E2, (E3, R4), (E4, R3), (NE, CC), (N1, R2)),
        N1: (N2, (N3, G1), (N4, G2), (NW, CC), (W1, G3)),
        W1: (W2, W3, W4, SW, S1),
        S1: (S2, S3, S4, SE, WON),

        # straight-line clockwise 2s # 시계 방향 2로 이동
        E2: ((E3, R4), (E4, R3), (NE, CC), (N1, R2), (N2, R1)),
        N2: ((N3, G1), (N4, G2), (NW, CC), (W1, G3),(W2, G4)),
        W2: (W3, W4, SW, S1, S2),
        S2: (S3, S4, SE, WON, WON),

        # straight-line clockwise 3s 시계 방향 3으로 이동
        E3: (E4, NE, N1, N2, (N3, G1)),
        N3: (N4, NW, W1, W2, W3),
        W3: (W4, SW, S1, S2, S3),
        S3: (S4, SE, WON, WON, WON),

        # straight-line clockwise 4s # 시계 방향 4로 이동
        E4: (NE, N1, N2, (N3, G1), (N4, G2)),
        N4: (NW, W1, W2, W3, W4),
        W4: (SW, S1, S2, S3, S4),
        S4: (SE, WON, WON, WON, WON),

        # straight-line clockwise 5s # 시계 방향 5로 이동
        NE: (N1, N2, (N3, G1), (N4, G2), (NW, CC)),
        NW: (W1, W2, W3, W4, SW),
        SW: (S1, S2, S3, S4, SW),
        SE: (WON, WON, WON, WON, WON),
